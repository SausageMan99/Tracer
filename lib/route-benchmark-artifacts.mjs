...
function roundScore(value) {
  return Number(Math.max(0, Math.min(1, value ?? 0)).toFixed(5));
}
...
function sumEdgesKm(edges, predicate) {
  return edges
    .filter(predicate)
    .reduce((sum, edge) => sum + (edge.lengthKm ?? 0), 0);
}

function bestCandidateFrom(route) {
  return route?.best ?? (Array.isArray(route?.candidates) ? route.candidates[0] : null) ?? route ?? null;
}

function naturalDwellKmFrom(candidate, edges) {
  const qualityValue = candidate?.quality?.naturalZoneDwellKm;
  if (Number.isFinite(qualityValue)) return roundKm(qualityValue);
  const distanceKm = candidate?.distanceKm ?? 0;
  if (Number.isFinite(candidate?.quality?.naturalWayRatio) && distanceKm > 0) {
    return roundKm(candidate.quality.naturalWayRatio * distanceKm);
  }
  return roundKm(sumEdgesKm(edges, (edge) => edge.flags?.natural === true));
}

function targetComponentDwellKmFrom(candidate, edges, targetComponentIds) {
  const qualityValue = candidate?.quality?.targetComponentDwellKm;
  if (Number.isFinite(qualityValue)) return roundKm(qualityValue);
  if (targetComponentIds.size === 0) return 0;
  return roundKm(sumEdgesKm(edges, (edge) => targetComponentIds.has(edge.componentId) && edge.flags?.natural === true));
}

function actualConnectorKmFrom(candidate, edges, targetComponentIds) {
  const pavedKm = Number.isFinite(candidate?.quality?.pavedRatio) && Number.isFinite(candidate?.distanceKm)
    ? candidate.quality.pavedRatio * candidate.distanceKm
    : sumEdgesKm(edges, (edge) => edge.flags?.paved === true);
  const targetPavedKm = targetComponentIds.size === 0
    ? 0
    : sumEdgesKm(edges, (edge) => targetComponentIds.has(edge.componentId) && edge.flags?.paved === true);
  const nonTargetRoadLikeKm = sumEdgesKm(edges, (edge) => !targetComponentIds.has(edge.componentId) && (edge.flags?.paved === true || edge.flags?.busy === true));
  return roundKm(Math.max(0, Math.max(pavedKm - targetPavedKm, nonTargetRoadLikeKm)));
}

function promiseHonestyScore(actualOutcome, realisticOutcome, candidate) {
  const pavedRatio = candidate?.quality?.pavedRatio ?? 0;
  const trailRatio = candidate?.quality?.trailRatio ?? 0;
  if (actualOutcome === "typed_refusal" || actualOutcome === "refused") {
    return realisticOutcome === "refused" ? 1 : 0.45;
  }
  if (actualOutcome === "adjusted_distance" || actualOutcome === "best_effort_route" || actualOutcome === "adjusted") {
    return realisticOutcome === "adjusted" ? 1 : realisticOutcome === "refused" ? 0.8 : 0.75;
  }
  if (realisticOutcome === "refused") return 0.25;
  if (realisticOutcome === "adjusted") return 0.7;
  if (pavedRatio > 0.7 && trailRatio < 0.12) return 0.45;
  return 1;
}

export function routeToOpportunityCaptureMetrics(benchmark, route, terrainOpportunityReport = routeToTerrainOpportunityReport(benchmark, route)) {
  if (terrainOpportunityReport == null) return null;
  const candidate = bestCandidateFrom(route);
  if (candidate == null) return null;
  const edges = Array.isArray(candidate.edgeDiagnostics) ? candidate.edgeDiagnostics : [];
  const targetComponentIds = new Set(terrainOpportunityReport.strategy?.targetComponents ?? []);
  const bestAvailable = terrainOpportunityReport.bestAvailable ?? {};
  const routeDistanceKm = candidate.distanceKm ?? 0;
  const naturalDwellKm = naturalDwellKmFrom(candidate, edges);
  const continuousNaturalKm = roundKm(candidate?.quality?.longestNonPavedTrailStreakKm ?? candidate?.quality?.longestTrailSegmentKm ?? 0);
  const targetComponentDwellKm = targetComponentDwellKmFrom(candidate, edges, targetComponentIds);
  const actualConnectorKm = actualConnectorKmFrom(candidate, edges, targetComponentIds);
  const feasibleNaturalDwellKm = bestAvailable.maxNaturalDwellKm ?? 0;
  const feasibleContinuousNaturalKm = bestAvailable.maxContinuousNaturalKm ?? 0;
  const feasibleTargetDwellKm = targetComponentIds.size > 0
    ? terrainOpportunityReport.components
        .filter((component) => targetComponentIds.has(component.id))
        .reduce((sum, component) => sum + (component.estimatedDwellCapacityKm ?? 0), 0)
    : feasibleNaturalDwellKm;
  const minConnectorKm = bestAvailable.minConnectorKmToUsefulTerrain ?? null;
  const necessaryConnectorBudgetKm = minConnectorKm == null ? 0 : minConnectorKm * 2;
  const naturalDwellCaptureRatio = feasibleNaturalDwellKm > 0 ? clampRatio(naturalDwellKm / feasibleNaturalDwellKm) : 0;
  const continuousNaturalCaptureRatio = feasibleContinuousNaturalKm > 0 ? clampRatio(continuousNaturalKm / feasibleContinuousNaturalKm) : 0;
  const targetComponentCaptureRatio = feasibleTargetDwellKm > 0 ? clampRatio(targetComponentDwellKm / feasibleTargetDwellKm) : 0;
  const connectorEfficiencyRatio = minConnectorKm == null
    ? 1
    : actualConnectorKm <= 0
      ? (minConnectorKm <= 0 ? 1 : 0)
      : clampRatio(necessaryConnectorBudgetKm / actualConnectorKm);
  const avoidablePavementKm = roundKm(Math.max(0, actualConnectorKm - necessaryConnectorBudgetKm));
  const capturedComponentIds = new Set([
    ...(candidate?.quality?.visitedTargetComponents ?? []),
    ...edges.filter((edge) => (edge.lengthKm ?? 0) > 0.02).map((edge) => edge.componentId).filter(Boolean),
  ]);
  const capturedBestDwell = Math.max(targetComponentDwellKm, naturalDwellKm);
  const missedBetterComponentCount = terrainOpportunityReport.components.filter((component) => {
    if (capturedComponentIds.has(component.id)) return false;
    return (component.estimatedDwellCapacityKm ?? 0) > Math.max(0.5, capturedBestDwell * 0.8);
  }).length;
  const actualOutcome = route?.terrainFallback?.policy === "best_effort_terrain"
    ? "best_effort_route"
    : route?.distanceAdjustment?.policy === "adjusted_distance" || candidate?.distanceAdjustment?.policy === "adjusted_distance"
      ? "adjusted_distance"
      : "route_success";
  const honestyScore = promiseHonestyScore(actualOutcome, bestAvailable.realisticOutcome, candidate);
  const opportunityCaptureScore = roundScore(
    naturalDwellCaptureRatio * 0.28 +
    continuousNaturalCaptureRatio * 0.2 +
    targetComponentCaptureRatio * 0.22 +
    connectorEfficiencyRatio * 0.15 +
    honestyScore * 0.15 -
    Math.min(0.2, missedBetterComponentCount * 0.05) -
    Math.min(0.2, avoidablePavementKm / Math.max(routeDistanceKm, 1) * 0.8)
  );
  ...
  return {
    ...
    scoringBehaviorChanged: false,
    caseId: benchmark.id,
    opportunityCaptureScore,
    naturalDwellCaptureRatio: roundRatio(naturalDwellCaptureRatio),
    continuousNaturalCaptureRatio: roundRatio(continuousNaturalCaptureRatio),
    targetComponentCaptureRatio: roundRatio(targetComponentCaptureRatio),
    connectorEfficiencyRatio: roundRatio(connectorEfficiencyRatio),
    avoidablePavementKm,
    missedBetterComponentCount,
    promiseHonestyScore: roundScore(honestyScore),
    inputs: {
      routeDistanceKm: roundKm(routeDistanceKm),
      naturalDwellKm,
      feasibleNaturalDwellKm: roundKm(feasibleNaturalDwellKm),
      continuousNaturalKm,
      feasibleContinuousNaturalKm: roundKm(feasibleContinuousNaturalKm),
      targetComponentDwellKm,
      feasibleTargetDwellKm: roundKm(feasibleTargetDwellKm),
      actualConnectorKm,
      necessaryConnectorBudgetKm: roundKm(necessaryConnectorBudgetKm),
      realisticOutcome: bestAvailable.realisticOutcome ?? null,
      actualOutcome,
    },
    approximationNotes: [
      "Opportunity-capture metrics are benchmark-observation only; they do not alter engine scoring or route selection.",
      "Connector and avoidable pavement are approximate until the planner exposes explicit connector paths.",
    ],
  };
}

export function routeToOpportunityCaptureArtifact(benchmark, route) {
  const terrainOpportunityReport = routeToTerrainOpportunityReport(benchmark, route);
  const opportunityCaptureMetrics = routeToOpportunityCaptureMetrics(benchmark, route, terrainOpportunityReport);
  if (terrainOpportunityReport == null && opportunityCaptureMetrics == null) return null;
  return {
    schemaVersion: 1,
    observationOnly: true,
    scoringBehaviorChanged: false,
