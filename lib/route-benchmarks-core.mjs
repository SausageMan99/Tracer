...
function shapeIssueSeverity(key, actual, limit, direction) {
  if (key === "self_intersection" && direction === "max" && limit === 0) {
    return actual >= 2 ? 2 : 1;
  }
...
function addShapeIssue(issues, key, actual, limit, direction, weight = 1) {
  ...
  if (!failed) return;
  const severity = shapeIssueSeverity(key, actual, limit, direction);
