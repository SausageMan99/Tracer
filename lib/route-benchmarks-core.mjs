...
export function summarizeBenchmarkFailure(benchmark, payload) {
  const failures = [];
  const expectsTypedRefusal = benchmark.expectedOutcome === "typed_refusal" || benchmark.expectedOutcome === "park_recovery";
...
export function summarizeBenchmarkResult(benchmark, route, durationMsOverride) {
  ...
  const expectsAdjustedDistance = benchmark.expectedOutcome === "adjusted_distance" || benchmark.expectedOutcome === "park_recovery";
